...
exports.handler = async function (event, context) {
  ...
  try {
    ...
    if (body.callback_query) {
      ...
      } else if (data === 'status_enrolled') {
        newText = originalText.replace(
          /Status: .*/g,
          "Status: 🎓 Ro'yxatdan o'tdi",
        );
      ...
      const inlineKeyboard = {
        inline_keyboard: [
          ...
          [
            { text: "🎓 Ro'yxatdan o'tdi", callback_data: 'status_enrolled' },
    ...
    // --- Saytdan yangi ariza ---
      ...
      const { name, phone, age, course = 'Matematik Kurslari' } = body;
      ...
      // Voucher kodi yaratish
      const voucherCode = `MATH${new Date().getFullYear()}-${String(
        Math.floor(Math.random() * 1000),
      ).padStart(3, '0')}`;

      const messageText = `🧮 MATEMATIK KURSLARI - Yangi Ariza!
...
📅 Ariza vaqti: ${new Date().toLocaleString('uz-UZ')}
Status: ✅ Yangi

💡 *Bepul dars uchun voucher berildi!*`;
